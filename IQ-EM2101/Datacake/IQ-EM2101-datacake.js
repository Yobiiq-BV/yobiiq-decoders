 ...
 *                              
 * 
 * YOBIIQ JS payload decoder compatible with Datacake.
 * 
 ...
 * 
 *
 * @product     P1002009 iQ EM2101 STD (1-phase energy meter)
 * 
 * 
 */
...
function DATACAKE_FIELD_NAME(oldName)
{
    return oldName.replace(/([a-z])([A-Z])/g, '$1_$2').toUpperCase();
}
...
// Decode decodes an array of bytes into an array of objects. (Datacake)
//  - bytes is an array of bytes, e.g. [225, 230, 255, 0]
//  - fPort contains the LoRaWAN fPort number
// The function must return an array of objects, e.g. [{"field":"AMBIENT_TEMPERATURE", "value": 22.5}]
function Decoder(bytes, fPort)
{
    var decoded = Decode(fPort, bytes, null);
    var datacakeFields = [];
    var ts = null; // in seconds for datacake format
    if(typeof decoded.dataloggerTimestamp === "number")
    {
        ts = decoded.dataloggerTimestamp;
    }
    ...
    {
        key = keys[i];
        if (key === "dataloggerTimestamp")
        {
            continue; // skip this field
        }
        var datacakeRecord = {field: DATACAKE_FIELD_NAME(key), value: decoded[key]};
        if (ts !== null && key !== "timestamp")
        {
            // add timestamp to all fields except timestamp itself
            datacakeRecord.timestamp = ts;
        }
        datacakeFields.push(datacakeRecord);
    }
